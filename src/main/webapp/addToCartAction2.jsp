...
try{
	...
	ResultSet rs= st.executeQuery("select * from pizza_items where id='"+product_id+"'");
	while(rs.next())
	{
	...
	if(z==1)
	{
		...
		response.sendRedirect("cart.jsp?message=exist&");
	}
	if(z==0)
	{
		...
		ps.executeUpdate();
		response.sendRedirect("cart.jsp?message=added");
	}
}
catch(Exception e)
{
	System.out.println(e);
	response.sendRedirect("cart.jsp?message=invalid");
	}
%>